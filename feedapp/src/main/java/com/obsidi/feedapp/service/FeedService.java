...
import com.obsidi.feedapp.exception.domain.PageResponse;
                                ...
                                boolean alreadyLiked = feed.getFeedMetaData().stream()
                                                .anyMatch(
                                                                m -> m.getUser().getUsername().equals(username)
                                                                                && Boolean.TRUE.equals(m.getIsLike()));

                                if (alreadyLiked) {
                                        throw new LikeExistException(
                                                        String.format("Feed already liked, feedId: %d, username: %s",
                                                                        feedId, username));
                                }
        ...
        public void deleteFeed(int feedId) {
                ...
                Optional.of(feed).filter(f -> f.getUser().getUsername().equals(username))
                                .orElseThrow(() -> new FeedNotUserException(String
                                                .format("Feed doesn't belong to current User, feedId: %d, username: %s",
                                                                feedId, username)));
